...
def landing_page() -> str:
    ...
    <html lang="en">
      ...
      <body>
        ...
        <div class="note">
          <strong>Need automation?</strong> Run <code>scripts/run_api.sh</code> from the repository root to
          install dependencies and launch the server in one step.
        </div>
      </body>
    </html>
    """
...
# -------------------------------------------------------------------
# Debug: OTPs (only when debug_mode is True)
# -------------------------------------------------------------------
@app.get("/debug/otps")
def list_debug_otps(
...
def list_budgets(
    ...
    """
    Return budgets visible to the current user (admin sees all; others see their own).
...
def upload_budget_document(
    ...
    if not budget:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Budget not found")
    if budget.owner_id != current_user.id and current_user.role not in {
        UserRole.admin,
        UserRole.approver,
        UserRole.accounts,
    }:
